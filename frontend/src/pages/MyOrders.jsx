...
function MyOrders() {
  ...
  return (
    <div className='mx-2 shadow-md sm:mx-4 md:mx-16 lg:mx-32'>
        <p className='text-xl md:text-2xl mb-6 font-bold text-center md:text-start lg:ml-2'>My orders</p>
        {data.map((order,index)=>{
            return(
                <div key={index} className='grid grid-cols-3 lg:grid-cols-6  items-center border mb-4 pb-4 xl:pb-0 py-2 px-2  pl-4'>
                    <img className='h-12 ml-2 md:ml-0  md:h-14 '  src={assets.parcel_icon} alt="" />
                    <p className='text-sm  w-28 md:36 lg:w-44  text-gray-600'>{order.items.map((item,index)=>{
